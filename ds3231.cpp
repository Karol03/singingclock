...
#define SECONDS_FROM_1970_TO_2000 946684800llu
...
static uint64_t date2days(uint64_t y, uint8_t m, uint8_t d) {
    if (y >= 2000)
        y -= 2000;
    uint64_t days = d;
    ...
    if (m > 2 && isleapYear(y))
        ++days;
    return days + 365llu * y + (y + 3llu) / 4llu - 1llu;
}

static uint64_t time2longlong(uint64_t days, uint64_t h, uint64_t m, uint64_t s) {
    return ((days * 24llu + h) * 60llu + m) * 60llu + s;
}
...
DateTime::DateTime (uint64_t t) {
  t -= SECONDS_FROM_1970_TO_2000;    // bring to 2000 timestamp from 1970
    ...
    t /= 60llu;
    hh = t % 24llu;
    uint64_t days = t / 24llu;
    uint8_t leap;
    for (yOff = 0; ; ++yOff) {
        leap = isleapYear((uint16_t) yOff);
        if (days < (uint64_t)(365llu + leap))
            break;
        days -= (365llu + leap);
    }
    for (m = 1; ; ++m) {
...
uint64_t DateTime::unixtime(void) const {
  uint64_t t;
  uint64_t days = date2days(yOff, m, d);
  t = time2longlong(days, hh, mm, ss);
...
DateTime RTClib::now(TwoWire & _Wire) {
  ...
  if (_Wire.write(0) == 0)	// This is the first register address (Seconds)
...
void DS3231::setEpoch(time_t epoch, bool flag_localtime) {
	...
	setDoW((tmnow.tm_wday + 6) % 7 + 1);  // tm_wday is day of week but in range [0, 6] (where 0 is sunday(!))
